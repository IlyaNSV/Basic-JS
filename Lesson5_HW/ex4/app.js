...
function handleClick(clickedButtonEvent) {
    ...
    let inner_text = card.button.innerText;
    
    if (inner_text === "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (inner_text === "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    card.productName.insertAdjacentHTML("afterend", '<div class="desc">There must be some description text text</div>');
